...
public class PageDrawworksLimitChecks extends Page
{

    public PageDrawworksLimitChecks()
    {
    ...
    public ReportBean build(Connection connection, HttpServletRequest request) throws Exception
    {
        ...
        PreparedStatement stmt = AlarmsDB.getDrawworksAlarms(connection, userBean);
        ...
        while (rs.next())
        {
            ...
            if (record.getDescription().contains("Floor") || record.getDescription().contains("Crown"))
            {
                if (!record.getDescription().contains("Brakes released"))
                {
                    histogram.increment(columnName, alarmType, 1);
                }
            } else if (alarmType.getDescription().contains("Brake"))
            {
                AlarmTypeBean typeMachineStop = new AlarmTypeBean();
                typeMachineStop.setSystem(alarmType.getSystem());
                typeMachineStop.setSubSystem(alarmType.getSubSystem());
                typeMachineStop.setMessageType(alarmType.getMessageType());
                typeMachineStop.setPriority(alarmType.getPriority());
                typeMachineStop.setTagName(alarmType.getSubSystem());
                typeMachineStop.setDescription("Total Brake Alarms");
        ...
        ReportBean reportBean = new ReportBean("/DrawworksLimitChecks.jsp", getName(userBean.getLabelsBean()));
    ...
    {
        return labelsBean.getLimit_checks();
    }
